...
While that's going on, open up \href{https://www.overleaf.com/learn}{\color{blue}\underline{Overleaf}} and check it out.  This website is a very good source on basic \LaTeX, and is very good to have open as you work.
...
One thing you may want to do is add a full-page cover.  If you do, before the call to maketitle, add a fullpageimage command, referencing your cover (see \hyperref[Adding Images]{Adding Images}).
...
\begin{tabular}{|cc|}
    \hline
    \CCKinetic   & \verb|\CCKinetic{}|   \\\hline
    \CCEnergy    & \verb|\CCEnergy{}|    \\\hline
    \CCExplosive & \verb|\CCExplosive{}| \\\hline
    \CCBurn      & \verb|\CCBurn{}|      \\\hline
    \CCHeat      & \verb|\CCHeat{}|      \\\hline
    \CCVariable  & \verb|\CCVariable{}|  \\\hline
\end{tabular}

\paragraph{Attack Symbols}
\begin{tabular}{|cc|}
    \hline
    \CCThreat     & \verb|\CCThreat{}|     \\\hline
    \CCRange      & \verb|\CCRange{}|      \\\hline
    \CCLine       & \verb|\CCLine{}|       \\\hline
    \CCBlast      & \verb|\CCBlast{}|      \\\hline
    \CCBurst      & \verb|\CCBurst{}|      \\\hline
    \CCCone       & \verb|\CCCone{}|       \\\hline
    \CCInvade     & \verb|\CCInvade{}|     \\\hline
    \CCAccuracy   & \verb|\CCAccuracy{}|   \\\hline
    \CCDifficulty & \verb|\CCDifficulty{}| \\\hline
\end{tabular}

\paragraph{CompCon Symbols}
\begin{tabular}{|cc|cc|}
    \hline
    \CCMovement    & \verb|\CCMovement{}|    & \CCHex        & \verb|\CCHex{}|        \\\hline
    \CCStructure   & \verb|\CCStructure{}|   & \CCStress     & \verb|\CCStress{}|     \\\hline
    \CCArmor       & \verb|\CCArmor{}|       & \CCHP         & \verb|\CCHP{}|         \\\hline
    \CCHollowHex   & \verb|\CCHollowHex{}|   & \CCCircle     & \verb|\CCCircle{}|     \\\hline
    \CCRepair      & \verb|\CCRepair{}|      & \CCCP         & \verb|\CCCP{}|         \\\hline
    \CCEffect      & \verb|\CCEffect{}|      & \CCSystem     & \verb|\CCSystem{}|     \\\hline
    \CCEvasion     & \verb|\CCEvasion{}|     & \CCEDEF       & \verb|\CCEDEF{}|       \\\hline
    \CCActivation  & \verb|\CCActivation{}|  & \CCGear       & \verb|\CCGear{}|       \\\hline
    \CCOvercharge  & \verb|\CCOvercharge{}|  & \CCOvershield & \verb|\CCOvershield{}| \\\hline
    \CCSystemPoint & \verb|\CCSystemPoint{}| & \CCDrone      & \verb|\CCDrone{}|      \\\hline
    \CCDeployable  & \verb|\CCDeployable{}|  & \CCDTwenty    & \verb|\CCDTwenty{}|    \\\hline
    \CCRoll        & \verb|\CCRoll{}|        &               &                        \\\hline
\end{tabular}

\paragraph{Mech Size Symbols}
\begin{tabular}{|cc|}
    \hline
    \CCSizeHalf  & \verb|\CCSizeHalf{}|  \\\hline
    \CCSizeOne   & \verb|\CCSizeOne{}|   \\\hline
    \CCSizeTwo   & \verb|\CCSizeTwo{}|   \\\hline
    \CCSizeThree & \verb|\CCSizeThree{}| \\\hline
\end{tabular}

\paragraph{NPC Class Symbols}
\begin{tabular}{|cc|}
    \hline
    \CCArtillery  & \verb|\CCArtillery{}|  \\\hline
    \CCBiological & \verb|\CCBiological{}| \\\hline
    \CCController & \verb|\CCController{}| \\\hline
    \CCDefender   & \verb|\CCDefender{}|   \\\hline
    \CCSupport    & \verb|\CCSupport{}|    \\\hline
    \CCStriker    & \verb|\CCStriker{}|    \\\hline
\end{tabular}

\paragraph{NPC Tier Symbols}
\begin{tabular}{|cc|}
    \hline
    \CCTierOne   & \verb|\CCTierOne{}|   \\\hline
    \CCTierTwo   & \verb|\CCTierTwo{}|   \\\hline
    \CCTierThree & \verb|\CCTierThree{}| \\\hline
\end{tabular}
...
\begin{tabular}[h]{|ll|}
    \hline
    \multicolumn{2}{|l|}{\large\textbf{IDENTITY}\hfill\normalsize\textcolor{gray}{\textbf{ROLL 1D20}}} \\
...
\LancerFrame[Manufacturer=IPS-N, Name=FAKEDRAKE, Size=2, Class=Defender, Class Icon=\CCDefender{}, Armor=3, HP=8, SP=5, Evasion=6, E-Defense=6, Heat Cap=5, Sensors=10, Tech Attack=+0, Repair Cap=5, Save Target=10, Speed=3, Background Image=IPS-N-Background, Facing Image=Drake-Facing]
    \end{lstlisting}
\end{minipage}
The first argument is the Frame stats, name, and, in truth, all the things that can be described in a single value.  The only two truly optional values are the Background Image and the Facing Image.  Background Image is a full screen image, placed behind everything like the company logos in the core book.  Facing Image is the optional image on the facing page, like the Drake in the core book.  It is optional because some mechs may not have art, and you simply want to start in on the License Levels.  If you don't want them, simply don't provide the argument.

Everything else is required. If you forget to put one of the values in, then the result will be a magenta colored box.  To demonstrate, the Speed option has been removed from the next page.  If you \textit{really} want to have a blank space, fill the argument with \verb|{}| instead.
...
scene.}{%No Flavor Text
    ...
    \end{lstlisting}
\end{minipage}
The Core System is marginally more complicated, but usually just because it has more content.  Insert the typical boxes and info boxes and such as needed to make up that content.
...
The first few arguments are the same as the LancerFrame command.  The Tiers are different.  At Tier 1, you need to specify all the values, or you get the same magenta box.  For Tiers 2 and 3, you only need to specify the values that change.
...
The next type is the full column image.  The command is \verb|\colimage{imgname}|, and it breaks the column (and possibly the page), fills the next, and then breaks again.  It scales the image to half the page width, so make your image's aspect ratio accordingly.  It also turns off page numbers, headers, and footers for the page, because they would draw over the image and look odd.  It will automatically detect which column it is in and
...
And last, the full page image.  \verb|\fullpageimage{imgname}|  This uses the entire width of the page, edge to edge, with no margins.  This is what you use for full-page art.  It triggers the new page commands, so you just put it where you want the break to be.

\fullpageimage{Intro-Last}

\newpage
The section after this is a duplication of the Lancer core book's Section 0.  The purpose is to demonstrate how you would replicate a larger section, formatting and all.  Obviously, at this length, the source code is not provided, so you'll need to check out the source on the github. TODO: Link this once created
\vfill
