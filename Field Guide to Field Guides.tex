...
One thing you may want to do is add a full-page cover.  If you do, before the call to maketitle, add a fullpageimage command, referencing your cover (see \hyperref[Images]{Adding Images}).
...
% \begin{tcolorbox}[colframe=red,colback=white,nobeforeafter,size=tight,boxrule=1mm]
...
\clearpage
\shipclass{Carriers}{blue}{This is where your flavor text goes.  Image credit JWST.  Thanks NASA!

    You create this by the command shipclass with four arguments. The title at the top of the page, the color of the first column, the flavor text under the image, and the background image.

    \lstinline{\\shipclass\{Carriers\}\{blue\}\{This is where your flavor text goes.  Image credit JWST.  Thanks NASA! <snip recursion>\}\{JWST-Jupiter\}}
}{JWST-Jupiter}
...
The Ship command is even simpler.  With only three stats, a few pieces of text, and an image, it's the shortest of all.  The infobox isn't even in the command, just following it, like NPCs.  The next text after the command gets inserted into the second column of the page.
\begin{lstlisting}
\LancerShip[Name=Tongass, Manufacturer=GMS, Points=4, HP=14, Defense=14, Options={2 Auxiliary, 2 Escorts}, Image=Tongass, Class=Line Carrier]
    {<snip flavor text>}
\infobox{orange}{\textbf{ \large Close Support}\\{\TechnicalText Trait}}
